...
*   **Why Random-One works:** Teissandier 2019 benchmarked multimapper handling in featureCounts and found that random-one integer assignment performs on par with fractional (1/n) for the family- and subfamily-level *aggregate* abundance, measured against simulated truth, while unique-only undercounts young families (grade A — see the grading table). That "on par" is scoped to the subfamily aggregate. It does not carry to the locus level or to differential expression — those are separate questions the benchmark does not address. The dominant TE tools instead reach for EM or fractional assignment (TEtranscripts/Telescope/SQuIRE use EM; TE-Seq drives Telescope fractionally); Random-One is our integer-compatible choice, chosen so the counts drop straight into DESeq2/edgeR without rounding.
*   **The gene and TE passes use different kernels.** The gene pass is unique-only (no `-M`), since a multimapper is genuinely unassignable to one gene; the TE passes are `-M` integer Random-One. These are different counting kernels, so the gene size factors are not the TE library scale — the basis of the gene-vs-TE caveat in §Combined and the kernel-mismatch check in [`QC.md`](./QC.md).
*   **Each fragment contributes exactly one count.** By construction under `--outSAMmultNmax 1`, each fragment emits exactly one locus while `NH` records its true multiplicity; direct inspection of the alignments confirms no fragment emits more than one locus. The delivered sense (`-s 2`) integer matrix is therefore fragment-weighted: young high-copy families are not alignment-inflated. An alignment-weighted run could instead have inflated the youngest families several-fold (in proportion to their mean multiplicity) — a risk averted by construction, not a realized effect. See [`QC.md` §9](./QC.md) (kernel tracer, fragment-weighting argument, counterfactual-risk pattern), which carries the alignment-weighting check.
...
For a stranded derepression study, though, the DE numerator is the **sense** channel; `-s 0` is kept only for QC reconciliation (the strand-split invariant), strictly dominated for DE by keeping sense and antisense separate — which reconstructs it while preserving the strand axis — and is never a DE numerator (grade C — inference).
...
(grade C — class-specific mechanism; see grading table). The full class-specific story is in 
...
That matters because several TE tools **bundle** their differential expression and lock it to a simple two-group comparison. TEtranscripts and SQuIRE run a fixed treatment-vs-control `~ condition` DESeq2 step; even TE-Seq, which is more flexible, ships DE configs that fit `~ [batch] + condition` (genes and TEs split post-fit; the subfamily aggregate via a `condition`-only glmmTMB) — interactions, factorial cells, and custom contrast vectors aren't wired in by default. To ask a richer question through those tools you end up running several pairwise jobs and stitching their count tables together by row id — a workaround, not a design.

Handing back a raw integer matrix removes that ceiling. The counts drop straight into edgeR / limma / DESeq2 with **any** design you can write — 2×2 factorial, interactions, blocking/batch, continuous covariates, random effects (`duplicateCorrelation` / `dream`) — and any `makeContrasts` you need. This design freedom is the toolkit's real contribution. In fairness it is not *unique* to featureCounts: atena's `SummarizedExperiment`, Telescope's counts, TEtranscripts' own `TEcount`, and SQuIRE's `--table_only` all emit matrices you can model yourself. The lock-in is specific to the bundled-DE wrappers — TEtranscripts and SQuIRE `Call`, which are two-group additive; Telescope, atena, `TEcount`, and SQuIRE `--table_only` are design-free in the same way this toolkit is. What this toolkit adds is a **clean integer** matrix — no rounding of fractional or EM weights — that feeds your model natively, paired with the strand sense/antisense channel and the QC gate.
...
This freedom can also become a burden, at the one point where keeping a flexible design alive would force you to rebuild a baked pipeline's machinery by hand. For *when* the crossover to a baked pipeline is the right call, see [`LIMITATIONS.md` §design freedom](./LIMITATIONS.md#design-freedom).
...
*   **Normalization (anchor size factors on genes) (grade B — contested; see grading table):** Size factors are best estimated from the **genes alone** — DESeq2 `estimateSizeFactors(dds, controlGenes = isGene)` (median-of-ratios on gene rows), or **equivalently in the edgeR/limma idiom** `calcNormFactors(method = "TMM")` computed on a genes-only `DGEList` with its `norm.factors` transplanted onto the combined object (`filterByExpr` applied *per feature type*, then `voom`/`voomLmFit` → `lmFit` → `eBayes`) — rather than pooled across genes+TEs. Same gene anchor, different estimator (median-of-ratios vs TMM). TE-Seq makes exactly this choice, with its verbatim `estimateSizeFactors(dds, controlGenes = rownames(dds) %in% gene_cts$gene_id)` and the comment "estimate the size factors using genes, and not RTEs" (`deseq.R:163-164`); the anchor is contested because TEtranscripts and SQuIRE instead pool genes and TEs into one DESeq estimate. The long-tailed, multimapper-inflated TE minority violates the "most features are unchanged" assumption that TMM/RLE rely on, and a pooled estimate can drag the gene log-fold-changes. Pooling genes+TEs into one size-factor estimate is safe only when the TE signal is modest. The benefit of combined mode is having the TEs *present in the same object* (one shared design and one FDR; the dispersion fit is split by feature type — see below), which is separate from putting them into the *size-factor estimate*. **Kernel caveat:** the gene matrix is unique-only (no `-M`) while the TE matrices are `-M` integer Random-One. These are different counting kernels. Gene-derived size factors are correct **for gene DE** (multimappers are genuinely unassignable to one gene), but the TE matrix's library scale includes `-M` TE signal the gene factors never saw ⇒ **gene size factors are not the TE library scale**; keep them off the TE matrix unless the mismatch is documented. Normalize TEs on a TE-appropriate (or spike/total) basis, or at minimum record the mismatch. This is the kernel-mismatch caveat — the operational check lives in [`QC.md`](./QC.md).
*   **The gene-anchored ruler assumes the gene compartment is mostly stable (grade B — see grading table).** That assumption can break. A transcriptome-wide response — a strong interferon/ISG program is the case in point — can shift gene composition enough that the gene-derived size factors move *with* the contrast. Then a TE fold-change is read against a ruler that is bending with the biology, and ruler and signal share a driver. So before you trust TE fold-changes in a strong-perturbation contrast, check that the gene-derived size factors are not themselves correlated with the contrast, and that they agree with a housekeeping subset. If they bend, anchor on an explicit stable-control set instead (`controlGenes` on housekeeping, or RUVg). The gene rows stay a valid *observed* control even when they are no longer a valid *ruler* — genes-as-control and genes-as-ruler are two different jobs.
*   **Dispersion — share the size factors, split the dispersion fit (grade A for the split; see grading table).** The genes supply a large, stable feature set for the shared, gene-anchored size factors. The dispersion trend is then best fit **separately** for the gene rows and the TE rows: the gene matrix is unique-only and the TE matrix is `-M` Random-One, so a single pooled mean-variance trend would fit the higher-variance young-TE rows to a trend estimated mostly on a different counting kernel. Share the size factors across the joint object; split the dispersion fit by feature type. This is what TE-Seq does — shared size factors on the joint object via `controlGenes`, then `DESeq()` run separately on the gene and TE subsets (`deseq.R:163-218`).
...
*   *Solution*: This toolkit assumes you have subtracted exon regions from the TE SAF (Simplified Annotation Format). This ensures no read is double-counted.
...
*   **Solution:** Remove TE SAF regions that overlap gene exons (as done in this toolkit's recommended workflow), or use tools like TEtranscripts, whose read-class priority sends **uniquely-mapped** reads gene-first while **multireads** go TE-first [[3]].
The gene-priority rule is therefore conditional, not flat: a *unique* read over a gene exon is withheld from the TE (to avoid attributing host gene expression to TEs), while a *multimapping* read over that same exon is preferentially assigned to the TE.

**Exon subtraction is double-counting hygiene, not autonomy isolation.** Removing exon-overlapping TEs keeps host *exon* (mature-mRNA) expression from being miscounted as TE; it says nothing about read-through, which is mostly *intronic*. Intronic TEs are not exonic, so they remain in the SAF and still catch host read-through. Isolating the autonomous-candidate signal — intergenic vs intronic — is genic-context stratification (Rung 2, [`LIMITATIONS.md`](./LIMITATIONS.md) §5), which the field handles by *labelling* TEs by context (e.g. TE-Seq's Exonic/Intronic/Intergenic), not by carving the annotation. (grade A — TE-Seq stratifies on genic context.) 
...
| **Combined Analysis** | Combined annotation (non-overlapping). | Gene-anchored size factors (DESeq2 `controlGenes` / edgeR genes-only TMM); dispersion fit split by feature type. Single design, one FDR. | Unified FDR. No double-counting. One shared, gene-anchored ruler. *(Valid for within-feature-type DE; not gene-vs-TE magnitude.)* | Complexity in prep. Requires checking overlaps; size factors must be gene-anchored, not pooled; dispersion split (genes vs TEs), not a single pooled trend. |
...
| Bundled DE locks the design (TEtranscripts/SQuIRE two-group; TE-Seq additive condition) | **A** | direct repo inspection: TEtranscripts/SQuIRE `~ condition`; TE-Seq shipped configs `~ [batch] + condition` (subfamily via `condition`-only glmmTMB) |
| Design freedom from a decoupled clean-integer matrix is not unique to featureCounts | **A** | repo inspection: atena `SummarizedExperiment`, Telescope counts, TEtranscripts' `TEcount`, SQuIRE `--table_only` also emit modellable matrices; the toolkit's add is the clean integer matrix + strand channel + QC gate |
| Matched strandedness for genes+TEs in single-pass tools | **B — mechanical default** | one global `--stranded` flag (TEtranscripts) |
...
### 2.0.3 — 2026-06-17 (boundaries clarity pass)
Documentation-only pass; no changes to `R/` or `scripts/` code. Applied the Rung-1 → Rung-2 boundaries findings:
- Normalization: added the gene-anchored-ruler audit caveat — the ruler assumes a mostly-stable gene compartment, a strong interferon/ISG program can bend it, so check the gene size factors are not correlated with the contrast (and agree with a housekeeping subset) before trusting TE fold-changes; fall back to an explicit stable-control set if they bend. Genes-as-control and genes-as-ruler are distinct jobs (grade B).
- Dispersion: downgraded the shared-dispersion-trend benefit. Stated the resolved posture — share the gene-anchored size factors across the joint object, then fit the dispersion trend separately for the gene rows and the TE rows (mixed kernel; as TE-Seq does). Updated the Combined row of the Summary Workflows table to match (grade A for the split).
- Design freedom: noted the design lock-in is specific to the bundled-DE wrappers (TEtranscripts, SQuIRE `Call`, two-group additive); Telescope, atena, `TEcount`, and SQuIRE `--table_only` are design-free. Added a pointer to `LIMITATIONS.md#design-freedom` for when freedom becomes a burden.
- Random-One: scoped the Teissandier "on par" result to the subfamily/family aggregate against simulated truth — it does not carry to locus level or differential expression.
- De-jargon: replaced the inline flag codes (kernel-mismatch, alignment-weighting) with the plain-language principle in prose; the operational flag checklist now ships beside the data (the dataset-side count-matrix manifest), not in the general docs.
